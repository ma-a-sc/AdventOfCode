...
class XMASfinder(object):
    ...
    def _prep_position_arrs(self) -> None:
        ...
        new_positions.append(
            [self._point(self.cur_pos.x + x, self.cur_pos.y) for x in range(4)]
        )
        # top
        ...
        # down
        new_positions.append(
            [self._point(self.cur_pos.x, self.cur_pos.y + x) for x in range(4)]
        )
        # top left
        arr = [
            self._point(self.cur_pos.x + x, self.cur_pos.y + x) for x in range(-3, 1)
        ]
        arr.reverse()
        new_positions.append(arr)
        # top right
        new_positions.append(
            [self._point(self.cur_pos.x - x, self.cur_pos.y + x) for x in range(4)]
        )
        # bottom left
        new_positions.append(
            [self._point(self.cur_pos.x + x, self.cur_pos.y - x) for x in range(4)]
